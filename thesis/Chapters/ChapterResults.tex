...
\end{tabular}%
}\caption{Average \ac{SSIM} index on 5 \ac{NIND}:\ac{X-T1} denoised scenes (ursulines-building, stefantiek, CourtineDeVillersDebris, MuseeL-Bobo, ursulines-red). The best performing models (to within two significant digits) are marked in bold.}
\label{tableNINDXT1}
\end{table*}
...
\end{tabular}%
}\caption{\ac{SSIM} index on \ac{NIND}:\ac{C500D} denoised set MuseeL-Bobo-C500D}
...
\end{tabular}%
}\caption{Average \ac{SSIM} index on 10 \ac{NIND}:\ac{C500D} scenes denoised with models trained on \ac{NIND}:\ac{X-T1} or with \ac{BM3D}}
...
\caption{Denoising performance of the MuseeL-Bobo-C500D set over increased ISO value (\ac{SSIM} values shown in Table \ref{tableNINDXT1onBoboC500D})}
\label{fig:boboc500d}
\end{figure}
...
\caption[Denoising stefantiek (visual comparison)]{Denoising stefantiek. 1: ISO200 ground-truth (1/20s), 2: high ISO (1/1900s), 3: 2 denoised using U-Net model trained with NIND, 4: 2 denoised using U-Net model trained with \ac{SIDD} (320-sets), 5: 2 denoised using \ac{BM3D} ($\sigma=90$\footnoteref{sigmanote})}
...
\caption[Denoising text present on sepiktext (visual comparison)]{Denoising text present on MuseeL-Sepik-C500D. 1: ISO200 ground-truth (2s), 2: high ISO (1/30s), 3: 2 denoised using U-Net model trained with NIND:X-T1, 4: 2 denoised using U-Net model trained with \ac{SIDD} (320-sets), 5: 2 denoised using BM3D ($\sigma=99$\footnoteref{sigmanote})}
\label{fig:text}
\end{figure*}
...
\caption[Denoising of an unpaired "high-speed" image]{Comparison between a noisy ISO6400 crop (top), one denoised with a model trained on \ac{NIND} (middle), and one on which \ac{BM3D} ($\sigma=30$) has been applied (bottom). Our model appears to perform well on dynamic scenes despite having been trained on static scenes.}
...
TODO (in-progress): HulbNet w/ HulDisc, HulbNet w/PatchGAN (done), HulbNet w/HulfDisc
