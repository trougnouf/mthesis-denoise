...
# The content of this script has been copied from denoise_image.py
# TODO modular combined script

parser = argparse.ArgumentParser(description='Image cropper with overlap')
parser.add_argument('--cs', default=320, type=int, help='Tile size (model was probably trained with 128, different values will work with unknown results)')
parser.add_argument('--ucs', default=256, type=int, help='Useful tile size (should be <=.75*cs), a smaller value may result in less grid artifacts but costs computation time')
...
parser.add_argument('-i', '--input', default='datasets/ValidationNoisyBlocksSrgb.mat', type=str, help='Input images .mat file')
parser.add_argument('-o', '--output', default='out.MAT', type=str, help='Output file with extension')
...
parser.add_argument('--model_subdir', type=str, default='2019-02-15T07:34_run_nn.py_--train_data_datasets-train-SIDDmed_128_96_datasets-train-NIND_128_96_--lossf_MSE_--lr_3e-4_--cuda_device_0_--time_limit_259200_--batch_size_94_--skip_sizecheck', help='subdirectory where .th models are saved (latest .th file is autodetected, models dir is assumed)')
...
class OneImageDS(Dataset):
    def __init__(self, inimg, cs, ucs, ol):
        self.inimg = Image.fromarray(inimg)
...
matd = sio.loadmat(args.input)
for s0 in range(matd['ValidationNoisyBlocksSrgb'].shape[0]):
    for s1 in range(matd['ValidationNoisyBlocksSrgb'].shape[1]):
        #inimg = (matd['ValidationNoisyBlocksSrgb'][s0][s1]/255).transpose(2,0,1)
        inimg = matd['ValidationNoisyBlocksSrgb'][s0][s1]
        ds = OneImageDS(inimg, args.cs, args.ucs, args.overlap)
        ...
        fswidth, fsheight = inimg.shape[0:2]
        ...
        matd['ValidationNoisyBlocksSrgb'][s0][s1] = (newimg.permute(1,2,0)*255).round()
        print('Elapsed time: '+str(time.time()-start_time)+' seconds')
matd['results'] = matd.pop('ValidationNoisyBlocksSrgb')
sio.savemat(args.output, matd)
